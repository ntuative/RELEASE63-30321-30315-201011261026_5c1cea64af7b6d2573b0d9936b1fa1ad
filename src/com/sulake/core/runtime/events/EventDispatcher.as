package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_859:uint = 2;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_1842 == param3)
               {
                  return;
               }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(_loc4_)
            {
               ...
               {
                  if(_loc6_.callback == param2 && _loc6_.var_1842 == param3)
                  {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!this._disposed)
         {
            ...
            if(this._result == const_859)
            {
               if(this.var_600 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + this.var_600.message,this.var_600.errorID,this.var_600);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,this.var_600);
               }
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         this._result = !!event.isDefaultPrevented() ? uint(const_1217) : uint(const_549);
      }
