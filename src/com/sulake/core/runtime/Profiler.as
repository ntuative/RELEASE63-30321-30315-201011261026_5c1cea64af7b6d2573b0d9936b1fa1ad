package com.sulake.core.runtime
{
   ...
   class Profiler extends Component implements IProfiler
   {
      ...
      {
         this.var_280 = new Dictionary(true);
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            for(_loc1_ in this.var_280)
            {
               IDisposable(this.var_280[_loc1_]).dispose();
               delete this.var_280[_loc1_];
            }
            super.dispose();
         }
      }
      ...
      {
         this.var_2628 = getTimer() - this.var_2212;
      ...
      public function getProfilerAgentsInArray() : Array
      {
         ...
         for(_loc3_ in this.var_280)
         {
            _loc2_ = _loc3_ as IUpdateReceiver;
            if(IDisposable(_loc3_).disposed)
            {
               IDisposable(this.var_280[_loc3_]).dispose();
               delete this.var_280[_loc3_];
            }
            else
            {
               _loc1_.push(this.var_280[_loc3_]);
            }
         }
         return _loc1_;
      }
      ...
      {
         var _loc2_:ProfilerAgent = this.var_280[param1];
