package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1053:IDropMenuWindow;
      
      private var var_450:ITextFieldWindow;
      
      private var var_612:Boolean = true;
      
      private var var_868:ICheckBoxWindow;
      
      private var var_1350:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_46.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_313));
         Logger.log("BEGINNING TO SHOW: " + this.var_313);
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_313);
         if(param1.flatId != this.var_313)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_313);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      {
         this.var_50 = IItemListWindow(this._frame.findChildByName("list_cont"));
         ...
         this.var_1053 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1053);
         this.var_1053.procedure = this.onSelectTemplate;
         this.var_868 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1350 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1349 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      {
         var _loc3_:IWindowContainer = IWindowContainer(this.var_50.getListItemByName(param2));
         ...
         if(!param1.exists)
         {
            this.var_50.removeListItem(_loc3_);
            this.var_50.removeListItem(this.var_50.getListItemByName("event_spacing"));
            return;
         }
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_46.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_46,WindowTracker.const_742,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_612 || this.var_450.text == "")
         {
            this.var_46.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_868.isSelected);
         this.var_46.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1477,_loc2_,this.var_450.text,"",""));
         if(this.var_1350.isSelected || this.var_1349.isSelected || this.var_868.isSelected)
         {
            this.var_46.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1350.isSelected,this.var_1349.isSelected,this.var_868.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1475);
         }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_46.initMsg.roomMessageTemplates[this.var_1053.selection];
         if(_loc3_ != null)
         {
            this.var_612 = false;
