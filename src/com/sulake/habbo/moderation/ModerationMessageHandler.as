package com.sulake.habbo.moderation
{
   ...
   public class ModerationMessageHandler
   {
      ...
      {
         if(param1 == null || this.var_58 == null)
         {
            return;
         }
         ...
         this.var_58.issueManager.playSound(_loc3_);
         this.var_58.issueManager.updateIssue(_loc3_);
         this.var_58.issueManager.updatePriorities();
      }
      ...
      private function onModeratorInit(param1:ModeratorInitMessageEvent) : void
      {
         ...
         {
            this.var_58.issueManager.updateIssue(_loc6_);
         }
         this.var_58.issueManager.updatePriorities();
         this.var_58.initMsg = _loc3_;
         this.var_58.startPanel.show();
      }
      ...
      private function onIssuePickFailed(param1:IssuePickFailedMessageEvent) : void
      {
         ...
         var pickedAlready:Boolean = this.var_58.issueManager.issuePickFailed(issues);
         if(pickedAlready)
         {
            if(retryEnabled)
            {
               ...
               {
                  alert = false;
                  this.var_58.issueManager.autoPick(retryEnabled,retryCount);
               }
            }
         }
         if(alert)
         {
            this.var_58.windowManager.alert("Error","Issue picking failed",0,function(param1:IAlertDialog, param2:Event):void
            {
               param1.dispose();
            });
         }
      ...
      private function onIssueDeleted(param1:IssueDeletedMessageEvent) : void
      {
         ...
         this.var_58.issueManager.removeIssue(_loc2_.issueId);
      }
      ...
      private function onUserInfo(param1:ModeratorUserInfoEvent) : void
      {
         ...
         for each(_loc3_ in this.var_1048)
         {
      ...
      private function onRoomInfo(param1:ModeratorRoomInfoEvent) : void
      {
         ...
         for each(_loc3_ in this.var_1045)
         {
      ...
      private function onCfhChatlog(param1:CfhChatlogEvent) : void
      {
         ...
         this.onChatlog("Call For Help Chatlog: " + _loc2_.data.chatRecordId,WindowTracker.const_1028,_loc2_.data.callId,_loc3_,_loc4_);
      }
      ...
      private function onUserChatlog(param1:UserChatlogEvent) : void
      {
         ...
         this.onChatlog("User Chatlog: " + _loc2_.data.userName,WindowTracker.const_1088,_loc2_.data.userId,_loc2_.data.rooms,_loc3_);
      }
      
      private function onChatlog(param1:String, param2:int, param3:int, param4:Array, param5:Dictionary) : void
      {
         var _loc6_:* = null;
         for each(_loc6_ in this.var_1046)
         {
      ...
      private function onRoomVisits(param1:RoomVisitsEvent) : void
      {
         ...
         for each(_loc3_ in this.var_1047)
         {
      ...
      private function onRoomEnter(param1:RoomEntryInfoMessageEvent) : void
      {
         ...
         for each(_loc3_ in this.var_835)
         {
            _loc3_.onRoomChange();
         }
      }
      ...
      private function onRoomExit(param1:CloseConnectionMessageEvent) : void
      {
         ...
         for each(_loc2_ in this.var_835)
         {
            _loc2_.onRoomChange();
         }
      }
      ...
      private function onModeratorActionResult(param1:ModeratorActionResultMessageEvent) : void
      {
         ...
         if(_loc2_.success)
         {
            this.var_58.connection.send(new GetModeratorUserInfoMessageComposer(_loc2_.userId));
         }
         else
         {
            this.var_58.windowManager.alert("Alert","Moderation action failed",0,this.onAlertClose);
         }
      }
      ...
      public function removeChatlogListener(param1:ChatlogCtrl) : void
      {
         var _loc3_:* = null;
         var _loc2_:Array = new Array();
         for each(_loc3_ in this.var_1046)
         {
            if(_loc3_ != param1)
            {
               _loc2_.push(_loc3_);
