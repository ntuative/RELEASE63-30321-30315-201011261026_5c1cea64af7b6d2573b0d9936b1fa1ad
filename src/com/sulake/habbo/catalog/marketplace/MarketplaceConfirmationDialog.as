package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketplaceConfirmationDialog implements IGetImageListener
   {
      ...
      public function showConfirmation(param1:int, param2:MarketPlaceOfferData) : void
      {
         ...
         this.var_80 = param2;
         if(!this.var_741 || !this._catalog || !this._catalog.localization)
         {
            return;
         }
         if(this._window)
         ...
         if(_loc3_)
         {
            if(param1 == this.var_741.const_1684)
            {
               _loc3_.text = "${catalog.marketplace.confirm_header}";
            }
            if(param1 == this.var_741.const_1666)
            {
         ...
         if(_loc3_)
         {
            _loc3_.text = "${" + this.var_741.getNameLocalizationKey(param2) + "}";
         }
         ...
         if(_loc3_)
         {
            ...
            _loc4_ = _loc4_.replace("%price%",this.var_80.price);
            _loc3_.text = _loc4_;
         }
         ...
         if(_loc3_)
         {
            ...
            {
               _loc4_ = _loc5_.raw;
               _loc4_ = _loc4_.replace("%days%",this.var_741.averagePricePeriod.toString());
               _loc6_ = this.var_80.averagePrice == 0 ? " - " : this.var_80.averagePrice.toString();
         ...
         if(_loc3_)
         {
            ...
            {
               _loc4_ = _loc5_.raw;
               _loc4_ = _loc4_.replace("%count%",this.var_80.offerCount);
               _loc3_.text = _loc4_;
            }
            else
            {
      ...
      private function setImage() : void
      {
         var _loc1_:* = null;
         var _loc2_:* = null;
         if(!this.var_80 || !this._window || !this._roomEngine)
         {
            return;
         }
         if(!this.var_80.image)
         {
            if(this.var_80.furniType == 1)
            {
               _loc1_ = this._roomEngine.getFurnitureIcon(this.var_80.furniId,this);
            }
            else if(this.var_80.furniType == 2)
            {
               _loc1_ = this._roomEngine.getWallItemIcon(this.var_80.furniId,this);
            }
            if(_loc1_ && _loc1_.data)
            {
               this.var_80.image = _loc1_.data as BitmapData;
               this.var_80.imageCallback = _loc1_.id;
            }
         }
         if(this.var_80.image != null)
         {
            ...
            if(_loc2_)
            {
               ...
               _loc2_.bitmap.draw(this.var_80.image,new Matrix(1,0,0,1,(_loc2_.width - this.var_80.image.width) / 2,(_loc2_.height - this.var_80.image.height) / 2));
            }
         }
      }
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "buy_button":
               this._catalog.buyMarketPlaceOffer(this.var_80.offerId);
               this.hide();
               break;
      ...
      {
         if(this.var_80 && this.var_80.imageCallback == param1)
         {
            this.var_80.image = param2;
            this.setImage();
         }
      }
