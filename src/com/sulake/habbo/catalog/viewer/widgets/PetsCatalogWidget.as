package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_808 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_298 = [];
         this.var_552 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_808,this);
               if(_loc9_)
               {
                  this.var_552 = _loc9_.sellableBreeds;
                  this.var_298 = _loc9_.colors;
         ...
         while(_loc6_ < this.var_552.length)
         {
            ...
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_808,_loc10_.id),this.getRaceLocalizationKey(this.var_808,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_808,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_932,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_298)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_446 = param1.index;
         if(this.var_446 < 0 || this.var_446 > this.var_298.length)
         {
            this.var_446 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_446 >= 0 && this.var_446 < this.var_298.length)
         {
            _loc7_ = this.var_298[this.var_446] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_552[this.var_1553] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_808,_loc3_.id,_loc2_,AvatarScaleType.const_49,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_283,AvatarAction.const_793);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_446 >= this.var_298.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_298[this.var_446] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_552[this.var_1553] as IBreed;
