package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_956:BitmapData = null;
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_1955 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_1956 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_1954 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1443 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_188 != null && this.var_188.running)
         {
            this.var_188.reset();
            return false;
         }
         if(this.var_955 == 0)
         {
            this.var_955 = getTimer();
            return false;
         }
         if(this.var_1443 > 0 && this.var_1441 >= this.var_1443)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_955;
         if(_loc1_ > this.var_1956)
         {
            ++this.var_1441;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1191 = _loc2_.imageUrl;
         this.var_1953 = _loc2_.clickUrl;
         this.var_955 = getTimer();
         if(this.var_1191 != "")
         {
            ...
            {
               this.var_188 = new Timer(this.var_1954,1);
               this.var_188.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_188.running)
            {
               this.var_188.reset();
            }
            this.var_188.start();
            _loc4_ = new URLRequest(this.var_1191);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_502,0,0,param1.bitmapData,this.var_1953));
            this.var_955 = getTimer();
         }
         else
         {
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1192 = _loc2_.imageUrl;
         this.var_1442 = _loc2_.clickUrl;
         if(this.var_1192 != "" && this.var_1442 != "")
         {
            _loc3_ = this.var_1192;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1192);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_411,0,0,_loc4_,this.var_1442,this.var_956,this.var_954));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_956 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_954 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
