package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private static var var_1288:String = "%";
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1288);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1288,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            if(!this.var_1629)
            {
               if("false" == this.var_158.getKey("use.default.localizations","false"))
               {
                  _loc5_ = this.var_158.getKey("external.texts.txt","external_texts.txt");
                  events.addEventListener(LocalizationEvent.const_148,this.onLocalizationLoaded);
                  super.loadLocalizationFromURL(_loc5_);
               }
            }
            _loc4_ = 1;
            while(this.var_158.keyExists("localization." + _loc4_))
            {
               _loc6_ = this.var_158.getKey("localization." + _loc4_,"");
               _loc7_ = this.var_158.getKey("localization." + _loc4_ + ".name","");
               _loc8_ = this.var_158.getKey("localization." + _loc4_ + ".url","");
      ...
      {
         var _loc2_:* = null;
         events.removeEventListener(LocalizationEvent.const_148,this.onLocalizationLoaded);
         if(this.var_158.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_158.getKey("external.override.texts.txt");
            events.addEventListener(LocalizationEvent.const_148,this.onLocalizationOverrideLoaded);
      ...
      {
         events.removeEventListener(LocalizationEvent.const_148,this.onLocalizationOverrideLoaded);
         unlock();
      }
