package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_391.getValue(_loc4_) == null)
               {
                  this.var_391.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         if(param1 == null || this.var_485 == null)
         {
            return;
         }
         var _loc2_:int = this.var_485.indexOf(param1);
         if(_loc2_ != -1)
         {
            this.var_485.splice(_loc2_,1);
         }
      }
      ...
      {
         var _loc1_:* = null;
         while(this.var_485.length > 0)
         {
            _loc1_ = this.var_485.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_391.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      {
         var _loc4_:PetData = this.var_391.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_391.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_391.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_640.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1364.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_593.getValue(_loc6_) == null)
               {
                  ...
                  if(param3 != null && this.var_485.indexOf(param3) == -1)
                  {
                     this.var_485.push(param3);
                  }
               }
            }
         }
