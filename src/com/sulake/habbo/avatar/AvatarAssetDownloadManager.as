package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_893:int = 3;
      
      private var var_396:AssetLoaderStruct;
      ...
      private const const_1805:int = 100;
      
      private const const_1489:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_396 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_894 = new Timer(this.const_1805,1);
         this.var_894.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_396)
         {
            this.var_396.addEventListener(AssetLoaderEvent.const_31,this.onConfigurationComplete);
            this.var_396.addEventListener(AssetLoaderEvent.const_45,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_396)
         {
            this.var_396.removeEventListener(AssetLoaderEvent.const_31,this.onConfigurationComplete);
            this.var_396.removeEventListener(AssetLoaderEvent.const_45,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_893);
            if(this.var_1373.indexOf("?") > 0)
            {
               _loc2_ = this.var_1373 + "&retry=" + this.var_893;
            }
            else
            {
               _loc2_ = this.var_1373 + "?retry=" + this.var_893;
            ...
            this.var_396 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2485,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1114[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1771)
         {
            this.var_1113.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_485[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_485[_loc3_] = _loc6_;
            }
            this.var_895[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_895)
         {
            _loc7_ = true;
            _loc4_ = this.var_895[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_485[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_895[_loc3_];
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_710.length > 0 && this.var_711.length < this.const_1489)
         {
