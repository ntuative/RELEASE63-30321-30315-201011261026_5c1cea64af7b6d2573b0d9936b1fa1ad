package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1535:int = 60000;
       
      
      private var _structure:AvatarStructure;
      
      private var var_1222:IAvatarImage;
      ...
      private var var_596:AvatarCanvas;
      
      private var _disposed:Boolean;
      
      private var _bodyPartCache:AvatarImageBodyPartCache;
      
      private var var_331:IActiveActionData;
      
      private var var_365:AvatarImageBodyPartContainer;
      
      private var var_1371:String;
      
      private var var_146:AvatarImagePartContainer;
      ...
      private var var_2481:ImageData;
      
      private var var_1765:PaletteMap;
      
      private var var_890:Array;
      
      private var var_889:Point;
      ...
      {
         if(this.var_709 == param1)
         {
            return;
         }
         if(this.var_709 == GeometryType.const_887 && param1 == GeometryType.const_916 || this.var_709 == GeometryType.const_916 && param1 == GeometryType.const_887)
         {
            this.var_709 = param1;
            this.var_596 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         this.var_331 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_331;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_331 || !this.var_331.definition))
         {
            if(this.var_331.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_331.definition.state + "." + this.var_331.actionParameter);
               _loc13_ = param2 - this.var_331.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_331.overridingAction);
                  if(_loc14_ != null)
                  {
                     ...
                     {
                        this.var_331 = _loc14_.action;
                     }
                     if(_loc14_.type == AnimationLayerData.const_646)
                     {
                        if(_loc14_.action != null)
                        {
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1222.getFigure(),_loc5_,this.var_709,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_365 = _loc10_.getImageContainer(_loc4_);
         if(this.var_365 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_365 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_331);
            if(this.var_365 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_365,_loc4_);
         }
         this.var_365.offset = _loc8_;
         return this.var_365;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_596)
         {
            this.var_596 = this._structure.getCanvas(this._scale,this.var_709);
            if(!this.var_596)
            {
               return null;
            }
            this.var_2479 = new Point(0,this.var_596.height - int(this.var_596.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1390[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_622 || _loc10_ == AvatarAction.const_372;
         var _loc12_:int = param2.length - 1;
         _loc8_ = _loc12_;
         while(_loc8_ >= 0)
         {
            this.var_146 = param2[_loc8_] as AvatarImagePartContainer;
            this.var_1371 = this.var_146.bodyPartId;
            this.var_168 = this.var_146.partType;
            this.var_1766 = this.var_146.partId;
            this._frame = this.var_146.getFrameIndex(param3);
            _loc5_ = param1;
            _loc7_ = false;
            if(_loc6_)
            {
               ...
               else
               {
                  ...
                  if(this.var_146.flippedPartType != this.var_168)
                  {
                     this.var_168 = this.var_146.flippedPartType;
                  }
               }
            }
            this.var_1111 = this._scale + "_" + this.var_146.action.assetPartDefinition + "_" + this.var_168 + "_" + this.var_1766 + "_" + _loc5_ + "_" + this._frame;
            this.var_394 = this._assets.getAssetByName(this.var_1111) as BitmapDataAsset;
            if(this.var_394 == null)
            {
               this.var_1111 = this._scale + "_std_" + this.var_168 + "_" + this.var_1766 + "_" + _loc5_ + "_0";
               this.var_394 = this._assets.getAssetByName(this.var_1111) as BitmapDataAsset;
            }
            if(!this.var_394 || !this.var_394.content)
            {
               Logger.log("renderBodyPart could not find required asset: " + this.var_1111);
            }
            else
            {
               if(this.var_146.paletteMapId > -1)
               {
                  this.var_1765 = this._structure.renderManager.paletteManager.getPalette(this.var_146.paletteMapId);
                  if(this.var_1765 != null)
                  {
                     this.var_891 = this.var_394.content as BitmapData;
                     this.var_169 = new BitmapData(this.var_891.width,this.var_891.height);
                     this.var_2480 = this.var_1765.colorize(this.var_891.getPixels(this.var_891.rect));
                     this.var_169.setPixels(this.var_169.rect,this.var_2480);
                  }
                  else
                  {
                     this.var_169 = (this.var_394.content as BitmapData).clone();
                  }
               }
               else
               {
               ...
               if(_loc11_ && (this.var_1371 == AvatarBodyPartType.const_257 || this.var_1371 == AvatarBodyPartType.const_1237 || this.var_1371 == AvatarBodyPartType.const_1449) && this.var_146.color != null)
               {
                  _loc16_ = 0.7;
                  _loc17_ = this.var_146.color.colorTransform;
               ...
               else if(this.var_146.isColorable && this.var_146.color != null)
               {
                  this.var_169.colorTransform(this.var_169.rect,this.var_146.color.colorTransform);
               }
               if(this.var_146.isBlendable)
               {
                  this.var_169.colorTransform(this.var_169.rect,this.var_146.blendTransform);
               }
               this.var_889 = this.var_394.offset.clone();
               if(_loc7_)
               {
                  if(this._scale == AvatarScaleType.const_49)
                  {
               ...
               this.var_2481 = new ImageData(this.var_169,this.var_889,_loc7_);
               this.var_890.push(this.var_2481);
            }
            _loc8_--;
         }
         if(this.var_890.length == 0)
         {
            return null;
         }
         var _loc13_:ImageData = this.createUnionImage(this.var_890,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2479.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_49)
            {
         ...
         _loc8_ = this.var_890.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_890.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
